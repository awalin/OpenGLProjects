...
-(void) initBars{
    ...
    for(int i=0;i<totalBars; i++){
        ...
        GLfloat x1 = radius*sin(x)*cos(y);
        GLfloat y1 = radius*sin(x)*sin(y);
        GLfloat z1 = radius*cos(y);
        location.roundLoc = GLKVector3Make( y1, z1, x1 );
        ...
        if(self.viewType==GLOBE){
            ...
            self.bars[count].colorCoords = GLKVector4Make(col.x/255, col.y/255, col.z/255, 1.0);
            self.bars[count].positionCoords = location.flatLoc;
            count++;
